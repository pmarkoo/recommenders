        ...
        with self:
            ...
            for _ in self:
                ...
                if (next_user != user) or self.next_row is None:
                    if not self.end_of_file and next_user in self.users:
                        raise FileNotSortedException(f"File {self.filename} is not sorted by user")
                    self.user2id[user] = len(self.user2id)
                if self.col_test_batch and (
                    (next_test_batch != test_batch) or self.next_row is None
                ):
                    if not self.end_of_file and next_test_batch < batch_index:
                        raise FileNotSortedException(
                            f"File {self.filename} is not sorted by {self.col_test_batch}"
                        )
                    batch_index += 1
            self.batch_indices_range = range(batch_index)
